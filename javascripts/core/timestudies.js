...
function canBuyStudy(name) {
  ...
  if (name == 53) {
      if (player.timestudy.studies.includes(43) && (player.eternities !== 0)) return true; else return false
  }
  if (name == 61) {
  ...
  if (name == 201) if(player.timestudy.studies.includes(192) && !player.timeless.upgrades.includes(8)) return true; else return false
  ...
  switch(row) {
      ...
      case 7:
      if (player.timeless.upgrades.includes(8)) {
...
function timeStudy21(){
	let ret=Decimal.pow(player.replicanti.amount, Math.pow(player.galaxies, .5)*0.05);
	return ret;
}
...
function timeStudy81(){
	let ret=3;
	if(player.timeless.upgrades.includes(7))ret=ret*1.5;
	return ret;
}
...
function timeStudy83(){
	let ret=Decimal.pow(1.0004, player.totalTickGained);
	if(!player.timeless.upgrades.includes(15))ret=ret.min(1e30);
	return ret;
}

function timeStudy91(){
	let ret=Decimal.pow(10, Math.min(player.thisEternity, 18000)/60);
	if(player.timeless.upgrades.includes(7))ret=ret.pow(1000);
	return ret;
}

function timeStudy92(){
	let ret=Decimal.pow(2, 600/Math.max(player.bestEternity, 20));
	return ret;
}

function timeStudy93(){
	let ret=Decimal.pow(player.totalTickGained, 0.5).max(1);
...
function timeStudy102(){
	let ret=Decimal.pow(5, player.replicanti.galaxies);
	if(player.timeless.upgrades.includes(11))ret=ret.pow(2);
	return ret;
}

function timeStudy103(){
	let ret=new Decimal(player.replicanti.galaxies).max(1);
	if(player.timeless.upgrades.includes(15))ret=ret.pow(100);
	return ret;
}

function timeStudy151(){
	let ret=new Decimal(1e4);
	if(player.timeless.upgrades.includes(5))ret=ret.mul(TLPU5());
	return ret;
}
