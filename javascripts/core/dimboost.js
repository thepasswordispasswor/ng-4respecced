...
function softReset(bulk) {
  ...
  player = {
      ...
      secondPow: getDimensionBoostPower().pow(player.resets-1).max(1),
...
function getDimboostCostIncrease () {
  ...
  if (player.timestudy.studies.includes(222)) ret -= 1
  return ret;
}
...
function getShiftRequirement(bulk) {
  ...
  if (player.currentChallenge == "challenge4") {
      ...
      if (tier == 6) amount += Math.max(player.resets+bulk - 2, 0) * mult+5;
  } else {
      tier = Math.min(player.resets + bulk + 4, 8)
  }

  if (tier == 8) amount += Math.ceil(Math.max(player.resets+bulk - 4, 0) * mult);
...
document.getElementById("timeSoftReset").onclick = function () {
  if ((!player.break && player.money.gt(Number.MAX_VALUE)) || player["timeDimension"+getTimeShiftRequirement(0).tier].bought+player["timeDimension"+getTimeShiftRequirement(0).tier].boughtAntimatter < getTimeShiftRequirement(0).amount) return;
	timeSoftReset(1)
};
...
function tickspeedSoftReset(bulk) {
  //if (bulk < 1) bulk = 1 (fixing issue 184)
  if (!player.break && player.money.gt(Number.MAX_VALUE)) return;
